...
import { ContextCompressor, CompressMessage } from './context-compressor.js';
...
describe('ContextCompressor', () => {
  ...
  describe('compress', () => {
    it('keeps at least 1 message verbatim', async () => {
      ...
      const msgs: CompressMessage[] = [{ role: 'user', content: 'hi' }];
    ...
    it('compresses 80% of messages', async () => {
      ...
      const msgs: CompressMessage[] = Array.from({ length: 10 }, (_, i) => ({
        role: 'user',
        content: `msg${i}`,
      }));
      const result = await compressor.compress(msgs);
    ...
    it('calls Haiku model', async () => {
      ...
      const msgs: CompressMessage[] = Array.from({ length: 5 }, (_, i) => ({
        role: 'user',
        content: `msg${i}`,
      }));
