...
1. _Share progress in the group_ via `mcp__agentlite__send_message` with a `sender` parameter matching their exact role/character name (e.g., `sender: "Marine Biologist"` or `sender: "Alexander Hamilton"`). This makes their messages appear from a dedicated bot in the Telegram group.
...
Also add `TELEGRAM_BOT_POOL` to the launchd plist (`~/Library/LaunchAgents/com.agentlite.plist`) in the `EnvironmentVariables` dict if using launchd.
...
>
> Check logs: `tail -f logs/agentlite.log | grep -i pool`
...
2. Check pool initialized: `grep "Pool bot" logs/agentlite.log`
...
8. Rebuild: `npm run build && ./container/build.sh && launchctl unload ~/Library/LaunchAgents/com.agentlite.plist && launchctl load ~/Library/LaunchAgents/com.agentlite.plist` (macOS) or `npm run build && ./container/build.sh && systemctl --user restart agentlite` (Linux)
