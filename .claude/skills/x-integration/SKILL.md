---
name: x-integration
description: X (Twitter) integration for AgentLite. Post tweets, like, reply, retweet, and quote. Use for setup, testing, or troubleshooting X functionality. Triggers on "setup x", "x integration", "twitter", "post tweet", "tweet".
---
...
> **Compatibility:** AgentLite v1.0.0. Directory structure may change in future versions.
...
1. **AgentLite is installed and running** - WhatsApp connected, service active
...
# Verify: launchctl list | grep agentlite (macOS) or systemctl --user status agentlite (Linux)
```
...
| `AGENTLITE_ROOT` | `process.cwd()`                                                | Project root directory                   |
...
| `logs/agentlite.log`      | Service logs (contains X operation logs) | Ignored |
...
To integrate this skill into AgentLite, make the following modifications:
...
All paths below are relative to project root (`AGENTLITE_ROOT`).
...
```bash
launchctl list | grep agentlite  # macOS — should show PID and exit code 0 or -
...
grep -i "x_post\|x_like\|x_reply\|handleXIpc" logs/agentlite.log | tail -20

# Script errors
grep -i "error\|failed" logs/agentlite.log | tail -20
```
...
docker run agentlite-agent ls -la /app/src/skills/
```
