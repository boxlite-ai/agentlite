...
| ---------------------- | -------------------------------------- | ----------------------------------------------- |
| **Main app logs**      | `logs/agentlite.log`                   | Host-side WhatsApp, routing, container spawning |
| **Main app errors**    | `logs/agentlite.error.log`             | Host-side errors                                |
...
echo '{}' | docker run -i \
  ...
  --entrypoint /bin/bash agentlite-agent:latest \
...
```bash
docker run --rm --entrypoint /bin/bash agentlite-agent:latest -c 'ls -la /workspace/'
```
...
```bash
docker run --rm --entrypoint /bin/bash agentlite-agent:latest -c '
  whoami
  ls -la /workspace/
  ls -la /app/
'
...
```bash
docker run --rm -it --entrypoint /bin/bash agentlite-agent:latest
```
...
# Also clear the session ID from AgentLite's tracking (stored in SQLite)
...
```bash
grep "Session initialized" logs/agentlite.log | tail -5
...
```bash
echo "=== Checking AgentLite Container Setup ==="
...
echo '{}' | docker run -i --entrypoint /bin/echo agentlite-agent:latest "OK" 2>/dev/null || echo "MISSING - run ./container/build.sh"
...
SESSIONS=$(grep "Session initialized" logs/agentlite.log 2>/dev/null | tail -5 | awk '{print $NF}' | sort -u | wc -l)
